            ...
            droppable.isOver && "ring-4 ring-primary ring-inset"
               ...
               {!droppable.isOver&&elements.length==0&&( 
                <p className='text-3xl text-muted-foreground flex flex-grow items-center font-bold'>
                    Arrastra y suelta los componentes de tu encuesta aqui
                </p>)}
                ...
                {elements.length>0&&(
                    <div className="flex flex-col  w-full gap-2 p-4">
                        {elements.map(element=>(
                            <DesignerElemementWrapper key={element.id} element={element}/>
                        ))}
                    </div>
                )}
            </div>
...
function DesignerElemementWrapper({element}:{element:FormElementInstance}) {
    ...
    const DesignerElement=FormElements[element.type].designerComponenet;
    return  (<div
    ...
    {mouseIsOver&&(
        ...
        <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 animate-pulse'>
            <p className='text-muted-foreground text-sm'>Haz Click para ver las propiedades o arrastra para mover</p>
        </div>
        </>
    )}
    {
        topHalf.isOver&&<div className='absolute top-0 w-full rounded-md h-[7px] bg-primary
        rounded-b-none'></div>
    }
    <div className={cn('flex w-full h-[120px] items-center rounded-md bg-accent/40 px-4 py-2 pointer-events-none opacity-100',
        ...
        {
        bottomHalf.isOver&&<div className='absolute bottom-0 w-full rounded-md h-[7px] bg-primary
        rounded-t-none'></div>
    }
    </div>);
}
